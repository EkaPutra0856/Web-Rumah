@foreach ($admin as $p)
    <div id="editModal{{ $p->id }}" class="hidden fixed inset-0 bg-gray-400 bg-opacity-60 justify-center items-center">
        <div class="bg-gray-800 rounded-lg w-1/2">
            <form method="POST" action="{{ url('/' . $p->id . '/update-administrator') }}" class="w-5/6 mx-auto my-5" onsubmit="return validateFormEdit('{{ $p->id }}')">
                @csrf
                ...
                <div class="basis-1/2 mb-5 px-2">
                    ...
                    <select name="gender" type="text" id="gender" required
                        class="border text-sm rounded-lg block w-full p-2.5 bg-gray-700 border-gray-600 placeholder-gray-400 text-white focus:ring-blue-500 focus:border-blue-500">
